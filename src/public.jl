...
arguments to ordinary functions. Keyword arguments to control executions are
  ...
  - `seconds::Real` Maximum amount of time to spend benchmarking. Defaults to `0.1` seconds
    ...
    running functions.
  - `gc::Bool` An expiremental option to disable garbage collection during benchmarking.
    ...
    the benchmark. As an expiremental option, this may be removed in the future or its
